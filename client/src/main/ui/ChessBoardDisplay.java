...
public class ChessBoardDisplay {
    ...
    public String highlight(ChessPosition startPosition) {
        var validMoves = game.validMoves(startPosition);
        Set<ChessPosition> validEndPositions = new HashSet<>();
        ...
        switch (playerColor) {
            case WHITE -> {
                ...
                for (int rowNum = 0; rowNum < rows.length; rowNum++) {
                    var row = rows[rowNum].toCharArray();
                    rowLabel(display, rowNum);
                    highlightBoardRowWhite(row, rowNum, display, validEndPositions);
                    rowLabel(display, rowNum);
                    display.append("\n");
                }
                colLabelsWhite(game, display);
            ...
            case BLACK -> {
                ...
                for (int rowNum = rows.length - 1; rowNum >= 0; rowNum--) {
                    var row = rows[rowNum].toCharArray();
                    rowLabel(display, rowNum);
                    highlightBoardRowBlack(row, rowNum, display, validEndPositions);
                    rowLabel(display, rowNum);
                    display.append("\n");
                }
                colLabelsBlack(game, display);
    ...
    private void highlightBoardRowWhite(char[] row, int rowNum, StringBuilder display, Set<ChessPosition> positionsToHighlight) {
        int colNum = 0;
        for (int i = row.length - 1; i >= 0; i--) {
            char token = row[i];
